...
@api_view(['GET', 'POST', 'PUT', 'DELETE'])
@permission_classes([IsAuthenticated])
@role_required(['admin', 'faculty'])  # Allow both admin and faculty roles
def course_view(request, course_id=None):
    ...
    # For POST, PUT, DELETE methods, explicitly check for admin role
    elif request.method in ['POST', 'PUT', 'DELETE']:
        # Explicitly check for admin role for modification methods
        ...
        if request.method == 'POST':
            try:
                ...
                if not course_id or not course_name:
                    ...
                    return Response({'error': 'Course ID and name are required'}, status=status.HTTP_400_BAD_REQUEST)
